...
class EqualsSpaceRemover:
    # https://stackoverflow.com/a/47265271/10530288
...
class Job:
    def __init__(self, args):
        ...
        for k in global_section:
            self._config.set('global', k, global_section[k])

        if args.zero_buffers:
            self._config.set('global', 'zero_buffers', None)
        # Windows does not support pthread mutexes, suppress the warning
        if os.name == 'nt':
            self._config.set('global', 'thread', None)
    ...
    def _create_jobfile(self, jobfile):
        if jobfile == '-':
            self._config.write(EqualsSpaceRemover(sys.stdout))
        else:
            with open(jobfile, 'w') as f:
                self._config.write(EqualsSpaceRemover(f))
    ...
    def create_job(self, rnd_type, queue_size, thread_num):
        ...
        for rw in ["read", "write", "rw"] if args.mix else ["read", "write"]:
            section_name = self._jobname_templ(job, rw)
            ...
            for k in section_options:
                self._config.set(section_name, k, section_options[k])
    ...
    def _info_get_bw_bytes(self, info):
        try:
            return info['bw_bytes'] / 10**6
        except KeyError:
            return info['bw'] / 10**3
    ...
    def _print(self, info):
        ...
        for job in self._jobs:
            self._printline(info, job, self._displayname(job), self._info_get_bw_bytes)
...
def get_parser():
    ...
    parser.add_argument('-a', metavar='job', dest='jobs', action="append", default=[],
